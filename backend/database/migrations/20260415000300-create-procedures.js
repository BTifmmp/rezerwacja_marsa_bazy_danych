...
module.exports = {
  async up(queryInterface) {
    await queryInterface.sequelize.query(`
      CREATE OR REPLACE PROCEDURE p_reserve(p_client_id INT, p_plot_id INT)
      LANGUAGE plpgsql
      AS $$
    ...
    await queryInterface.sequelize.query(`
      CREATE OR REPLACE PROCEDURE p_remove_reservation(p_client_id INT, p_plot_id INT)
      LANGUAGE plpgsql
      AS $$
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.query(`DROP PROCEDURE IF EXISTS p_remove_reservation(INT, INT);`);
    await queryInterface.sequelize.query(`DROP PROCEDURE IF EXISTS p_reserve(INT, INT);`);
  }
};
