...
module.exports = {
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.query(
      `DROP TRIGGER IF EXISTS trg_plot_reserved ON reservations;`,
    );
    await queryInterface.sequelize.query(
      `DROP TRIGGER IF EXISTS trg_client_balance ON payments;`,
    );
    await queryInterface.sequelize.query(
      `DROP TRIGGER IF EXISTS trg_update_payments ON reservations;`,
    );

    await queryInterface.sequelize.query(
      `DROP FUNCTION IF EXISTS f_update_reserved();`,
    );
    await queryInterface.sequelize.query(
      `DROP FUNCTION IF EXISTS f_update_user_balance();`,
    );
    await queryInterface.sequelize.query(
      `DROP FUNCTION IF EXISTS f_update_payments();`,
    );
  },
};
